...
Originally from Ashik/src/gridwise/replay.py (a942e15).
Aurna has the equivalent as `app/validator.py:replay_and_validate_schedule`;
Pulok inlines it inside the optimizer. Ashik's standalone `replay.py`
makes it unit-testable in isolation, which is what `tests/test_public_samples.py`
relies on for the per-case E_initial/E_final assertions.
