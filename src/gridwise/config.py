...
Originally from Ashik/src/gridwise/config.py (a942e15).
Merged-in from Aurna/app/config.py: masked_api_key() helper for safer logging.
"""
...
class Config:
    ...
    # ────────────────────── from Aurna/app/config.py ──────────────────────
    @classmethod
    def masked_api_key(cls, name: str) -> str:
        """Return a masked form of the named API key, safe to print.

        Merged in from Aurna/app/config.py:49. Returns '<unset>' if the
        key is empty, '<set>' if very short, otherwise `abc…yz`.
        """
        v = os.getenv(name)
        return _mask(v)

    @classmethod
    def all_keys_masked(cls) -> dict:
        """Return all known provider keys, masked. Safe for boot logs."""
        return {
            "GROQ_API_KEY": cls.masked_api_key("GROQ_API_KEY"),
            "GEMINI_API_KEY": cls.masked_api_key("GEMINI_API_KEY"),
            "OPENROUTER_API_KEY": cls.masked_api_key("OPENROUTER_API_KEY"),
            "PUKU_API_KEY": cls.masked_api_key("PUKU_API_KEY"),
        }
