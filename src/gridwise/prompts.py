...
Originally from Ashik/src/gridwise/prompts.py (a942e15).
Aurna has its own SYSTEM_PROMPT + FEW_SHOT_EXAMPLES inlined inside
app/llm_interpreter.py:24–125. Pulok's prompt is at src/interpreter.py and
is shorter (no few-shot examples). Ashik's factored approach with
build_user_prompt() as a separate function makes prompt regression tests
trivial; we keep it.
